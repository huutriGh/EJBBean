...
public class SampleIT {
    ...
    public static Archive<?> getEarArchive() {
        ...
        EnterpriseArchive ear = ShrinkWrap.create(EnterpriseArchive.class, "EJBBean-ear.ear");

        // Current directory is the root of the "EJBBean-web" project. Go up one level
        // and enter the "EJBBean-ejb" project.
        // The ejb jar is found in the "target" directory:
        File f = new File("../EJBBean-ejb/target/EJBBean-ejb.jar");
        JavaArchive ejbJar = ShrinkWrap.create(ZipImporter.class, "EJBBean-ejb.jar").importFrom(f).as(JavaArchive.class);
        ear.addAsModule(ejbJar);

        // Now grab the web archive:
        f = new File("./target/EJBBean-web.war");
        if (f.exists() == false) {
            throw new RuntimeException("File " + f.getAbsolutePath() + " does not exist.");
        }
        WebArchive war = ShrinkWrap.create(ZipImporter.class, "EJBBean-web.war").importFrom(f).as(WebArchive.class);
        ear.addAsModule(war);
